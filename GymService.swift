...
class GymService: ObservableObject {
    ...
    private init() {
        startPeriodicFetching()          // 15-min fetch for background / widgets
        setupAppLifecycleNotifications() // Start/stop 30-sec fetch for active app
    }
    
    // MARK: - Timers
    
    private func startPeriodicFetching() {
        Timer.publish(every: 900, on: .main, in: .common) // 15 minutes
            .autoconnect()
            .sink { [weak self] _ in
                Task {
